...
function generateMockData() {
  const buildingsData = buildings.map(b => {
    ...
    const anomaly = mlService.detectAnomaly(power, 'power');
    
    // HVAC fault detection
    const hvacFault = mlService.detectHVACFault({ power, occupancy, temperature });
    
    // Wastage calculation
    const wastagePercent = mlService.calculateWastage(power, b.baseline);
